...
class LearningModel {
  ...
  async initialize (model) {
    const nDef = await tf.loadModel(model || INITIAL_MODEL)
  ...
  async saveModel(done = () => {}) {
    await this.model.save('downloads://cardsModelTest-v-1');
    done('saved to ');
  }

  async loadModel(done = () => {}) {
    this.model = await tf.loadModel('/data/local/cardsModelTest-v-1.json');
    done('loaded');
  }
